...
from RGT.gridMng.models import Grid, GridChangeset, Alternatives, DiffType, Concerns, Ratings
...
def diff_creation_handler(sender, **kwargs):
    if kwargs.get('created'):
        ...
        GridChangeset.objects.ensure_initial_diff_exists(grid)
...
def __update_diff(instance, type, diff_count):
    ...
    diff, created = GridChangeset.objects.get_or_create(grid=grid, user=user, date=date.today(), type=type)
    if diff_count > 0:
        diff.added += 1
    elif diff_count < 0:
        diff.deleted += 1
