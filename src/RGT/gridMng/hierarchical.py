...
def drawDendogram(clusters= []):
        ...
        h= len(clusters[len(clusters) - 1][0]) + 2 * 100 # get the total number of elements of the last 'cluster' as it should contain all the elements
        w= 1200
        #depth= len(clusters)
...
#####--->>>>       #TODO calculate the max height of a word
        ...
        h= yTableTopOffset + (tableCellHeight * len(matrix)) + yAlternativeRulerOffset + ((len(matrix[0]) - 2) * tableCellHeight) + (alternativeMaxWordHeight * (len(matrix[0]) - 2)) + percentageWordSize[1] + yConcernRulerOffset + 10
        w= xWordTableOffset + (xWordToTableOffset * 2) + ((len(matrix[0]) - 2) * tableCellWidth) + leftConcernMaxWordWidth +  percentageWordSize[0]
        ...
        while i < len(temp):
            alternativeNameToIndex[temp[i]]= nCol - i #nCol - i is to invert the position of the alternative so the line that we will draw from the table to the alternative names will look good, also the result of the equation is the index for the new table, no correction is needed as the values of the alternatives start at index 1 
        ...
        while i < len(matrix):
            ...
            while j < len(matrix[0]) - 1:
                ...
                tempRow.pop(index + 1)
                if i == 1:
                    cellAlternativesXPositions[alternativeIndexToName[j]]= ((index - 1) * tableCellWidth) + (tableCellWidth / 2) + xWordTableOffset + xWordToTableOffset + leftConcernMaxWordWidth 
                j+= 1
        ...
        while i < len(matrix):
            ...
            if rightConcenrMaxX < xtableStartPosition + (tableCellWidth * (len(row) - 2)) +  xWordToTableOffset + tableLineThickness + rightConcernMaxWordWidth:
                rightConcenrMaxX= xtableStartPosition + (tableCellWidth * (len(row) - 2)) +  xWordToTableOffset + tableLineThickness + rightConcernMaxWordWidth
            j= 0
            i+= 1
        ...
        while i <  len(clustersAlternative[len(clustersAlternative) - 1][0]):
            allClusters.append(((clustersAlternative[len(clustersAlternative) - 1][0])[i], xtableStartPosition +  (tableCellWidth * (len(matrix[0]) -2) ) + xWordToTableOffset,  yTableStartPosition + (tableCellHeight * len(matrix)) + (tableCellHeight * (i)) + yAlternativeRulerOffset + percentageWordSize[1] + yAlternativePercentageToRulerOffset ))
            i+= 1
        ...
        yAlternativeRulerStartPoint= yAlternativeRulerOffset + percentageWordSize[1] + yAlternativePercentageToRulerOffset + yTableBottom
        #now reset all the x to the max word length + an offset
        temp= []
        for cluster in allClusters:
