        ...
        if len(self.data) > 0:
            ...
            for x in range(int(self.num_alternatives)):
                ...
                self.fields[alternativeName] = forms.CharField(
                    widget=forms.TextInput(attrs={'tabindex': '%d' % (x + 1), 'size': '30'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(self.data) > 0:
            ...
            for x in range(int(self.num_acrd)):
                ...
                # always adds '2-' % (where 2 the number of the step with zero index) prefix in the name,
                # with this the names are kept always the same.
                self.fields[acrd_name] = forms.CharField(widget=forms.HiddenInput(attrs={'class': 'acrd'}),
                                                         required=False)
        ...
        if len(self.data) > 0:
            ...
            for x in range(int(self.num_weights)):
                ...
                self.fields[weight_name] = forms.FloatField(
                    widget=forms.TextInput(attrs={'size': '3', 'maxlength': '3', 'tabindex': '%d' % (x + 1)}))
