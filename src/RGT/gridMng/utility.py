...
def convertGridTableToSvg(gridObj, concerns=None, alternatives=None, ratings=None):
    ...
    if gridObj != None:
        ...
        tempNode = xmlDoc.createRectNode(
            xTableOffSet + maxWidthLeftAndRightConcern + xLeftPoleCellOffset + 2 * xOffSetLeftAndRightPoleWordToCell,
            yTableOffset, tableLineThickness + cellHeight, (tableLineThickness * (nAlternatives + 1)) + sum(colWidths))
        ...
        while i < nConcerns:
            ...
            x = xLeftPoleCellOffset + maxWidthLeftAndRightConcern + (
                2 * xOffSetLeftAndRightPoleWordToCell) + xTableOffSet + tableWidth + xTableOffSet + (
                    maxWidthLeftAndRightConcern / 2) + xOffSetLeftAndRightPoleWordToCell - (size[0] / 2)
            y = yTableOffset + ((i + 1) * tableLineThickness) + ((i + 1) * cellHeight) + (tableLineThickness / 2) + (
                cellHeight / 2) + (size[1] / 2) - 5
        ...
        y = yTableOffset + ((nConcerns + 1) * cellHeight) + (
            (nConcerns + 2) * tableLineThickness) + yTotalWeightCellOffset
        height = size[1] + 10
        width = size[0] + 10
        tempNode = xmlDoc.createRectNode(x, y, height, width)
